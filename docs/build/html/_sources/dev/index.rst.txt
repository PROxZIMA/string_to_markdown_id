...
The String to Markdown Id project implements a regression
...
not broken existing functionality. The String to Markdown Id
...
``string_to_markdown_id`` package has been installed. If this is
not the case then you will likely need to set the ``PYTHONPATH`` environment
variable so that the ``string_to_markdown_id`` package can be found.
